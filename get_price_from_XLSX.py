...
wrkbk = openpyxl.load_workbook("2023 Biz Collection_NZ Pricelist Reseller.xlsx", data_only=True)
...
for i in range(2, 797):
    ...
    category_name = [" ", "Style", "STYLE", "WORKS PANTS & SHORTS", "ENGINEERED OUTERWEAR", "OVERALLS", "FIRE ARMOUR", "POLOS", "TEES", 
                     ...
                     "HEALTHWEAR", "CASUALWEAR", "TOPS", "SEPARATES"]
    ...
    price = sh.cell(row=i, column=7).value
    
    # This is so that there are no duplicates in the end list
    new_list_value = [pro_slug, price]
